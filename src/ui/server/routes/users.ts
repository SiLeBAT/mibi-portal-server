...
function getRouter(controllerFactory: IControllerFactory) {
	...
	router.use(
		'/register',
		getBaseRouter(RouterType.REGISTER, controllerFactory)
	);
	logger.verbose('Registering Route', { route: '/recovery' });
	router.use(
		'/recovery',
		getBaseRouter(RouterType.RECOVERY, controllerFactory)
	);
	...
	router.use(
		'/activate',
		getBaseRouter(RouterType.ACTIVATE, controllerFactory)
	);
	logger.verbose('Registering Route', { route: '/adminactivate' });
	router.use(
		'/adminactivate',
		getBaseRouter(RouterType.ADMINACTIVATE, controllerFactory)
	);
	return router;
}
