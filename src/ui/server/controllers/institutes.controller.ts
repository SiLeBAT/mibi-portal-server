...
export class DefaultInstituteController
    ...
    async getInstitutes(@request() req: Request, @response() res: Response) {
        ...
        const institutes = await this.redirectionTarget.get<InstituteCollectionDTO, AxiosResponse<InstituteCollectionDTO>, InstituteCollectionDTO>(INSTITUTES_ROUTE.ROOT).then((response) => {
