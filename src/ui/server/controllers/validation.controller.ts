...
class DefaultValidationController implements ValidationController {
    ...
    async validateSamples(req: Request, res: Response) {
        if (req.is('application/json')) {
            try {
                const sampleCollection: SampleCollection = this.fromDTOToSamples(
                    req.body
                );
                ...
                const validationResultsDTO = this.fromErrorsToDTO(
                    validationResult
                );
                logger.info(
                    'ValidationController.validateSamples, Response sent'
                );
                logger.verbose('Response:', validationResultsDTO);
                res.status(200).json(validationResultsDTO);
            } catch (err) {
                res.status(500).end();
                throw err;
            }
    ...
    private fromDTOToSamples(dto: ValidationRequest): SampleCollection {
        if (!Array.isArray(dto.data)) {
            throw new ApplicationSystemError(
                `Invalid input: Array expected, dto.data=${dto.data}`
            );
        }
