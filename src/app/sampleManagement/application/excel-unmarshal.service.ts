    ...
    private parseDate(stringOrDate: string | Date) {
        ...
        try {
            ...
            if (date.includes('GMT')) {
                const localOffset = moment().utcOffset();
                // parse as localized date of input timezone
                ...
                parsedMoment = this.correctMomentForXLSXSummerTimeBug(
                    parsedMoment,
                    new Date(date)
                );
                ...
                parsedMoment = moment(date, dateFormat);
            }

            if (!parsedMoment.isValid()) {
                return date;
            }
            return parsedMoment.locale('de').format('DD.MM.YYYY');
        } catch (e) {
            return date;
        }
    }
