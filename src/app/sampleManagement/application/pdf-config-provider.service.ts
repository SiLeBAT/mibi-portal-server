import { PDFConfigProviderService, PDFConstants, SampleSheetPDFConfig } from "../model/pdf.model";
import * as _ from 'lodash';
import { injectable, inject } from "inversify";
import { APPLICATION_TYPES } from "../../application.types";
import { SampleSheetConfig, SampleSheetMetaStrings, SampleSheetSamplesStrings, SampleSheetConstants } from "../model/sample-sheet.model";

@injectable()
export class DefaultPDFConfigProviderService implements PDFConfigProviderService {

    // mmToInch * inchToPixel
    private readonly MM_TO_PIXEL = 0.0393701 * 72;

    private readonly _config = { ..._.cloneDeep(this.constants.config), ..._.cloneDeep(this.pdfConstants.config) };
    private readonly _defaultStyle = _.cloneDeep(this.constants.defaultStyle);
    private readonly _styles = { ..._.cloneDeep(this.constants.styles), ..._.cloneDeep(this.pdfConstants.styles) };
    private _tableLayouts: {};

    public get config(): SampleSheetConfig & SampleSheetPDFConfig {
        return this._config;
    }
    ...
    public get tableLayouts(): {} {
        return this._tableLayouts;
    }

    public get strings(): { meta: SampleSheetMetaStrings, samples: SampleSheetSamplesStrings } {
        return { meta: this.constants.metaStrings, samples: this.constants.samplesStrings }
    }
    ...
    private preProcessMargins(margins: { left?: number, right?: number, top?: number, bottom?: number }, doScaling: boolean) {
    ...
    private createMetaTableLayout(): {} {
        ...
        return {
            ...
            ...this.createTableLayoutPaddingFunctions(this.config.meta.cellPadding)
        };
    }
    ...
    private createSamplesTableLayout(): {} {
        ...
        return {
            hLineWidth: (i: number, node: any) => {
                if (i === 0 || i === node.table.body.length || i === node.table.headerRows) {
            ...
            vLineWidth: (i: number, node: any) => {
                if (i === 0 || i === node.table.widths.length || doThickLines[i - 1]) {
                    return tableConfig.thickBorder;
                } else {
                    return tableConfig.thinBorder;
                }
            },
            ...this.createTableLayoutPaddingFunctions(this.config.samples.cellPadding)
        };
    }

    private createTableLayoutPaddingFunctions(padding: { left?: number, right?: number, top?: number, bottom?: number }): {} {
        let paddingOps = {};
